...
import userImage from './pp.png';

const userIcons = {
  Suresh: (
    <img src={userImage} alt="More options" className="dot-icon" />
  ),
  "Anoop sharma": (
    <img src={userImage} alt="More options" className="dot-icon" />
  ),
...
const KanbanColumn = ({ title, tickets, ticketCount, userProfilePic }) => {
  // Map title to priority label
  const mapTitleToPriority = (title) => {
    switch (title) {
      case "1":
        return "Low";
      case "2":
  ...
  const updatedTitle = mapTitleToPriority(title);
  ...
  return (
    <div className="kanban-column">
      ...
      <span className="user-icon">
        {userIcons[title] || null}
      </span>
      <span className="status-icon">
        {statusIcons[title] || null}
      </span>
      <span className="priority-icon">
        {priorityIcons[title] || null}
      </span>
          <span className="user-name">{updatedTitle}</span>
          <span className="ticket-count">{ticketCount}</span>
        </div>
        <div className="header-right">
          <span className="add-icon"><svg width="16" height="16" viewBox="0 0 16 16" fill="none" xmlns="http://www.w3.org/2000/svg">
